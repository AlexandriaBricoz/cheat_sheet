...
def sort(list):
    if any([isinstance(e,str) for e in list]):
    ...
    for namber in range(9,0,-1):
        ...
        for i, value in enumerate(list):
            if list[i]>=namber*(10**(n-1)):
                if int(str(list[i])[n]) == namber:
                    primer.append(value)
                    k = k+1
        if primer!=[]:
            n = n+1
            primer = [e for e in primer if e!=0 ]
...
if __name__ == '__main__':
    ...
    print(sort([12,23,342,90,9,99,967,30]))
    print(sort([1,111,11,2,12,10]))
